...
export class AuthService {
  ...
  async remove(id: number) {
    const status: any = await this.AuthModel.statusOfUser(id)
    console.log(status)
    if (status === 0) {
      throw new BadRequestException('User already deactivated')
    }

    const dellUser: any = await this.AuthModel.deleteAUser(id)
    if (!dellUser) throw new InternalServerErrorException('Error when try to deactivated this user')
  ...
  async active(id: number) {
    ...
    if (!activeUser) throw new InternalServerErrorException('Error when try to active this user')
